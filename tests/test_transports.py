...
from iso20022_evidence_pack_mcp import _cli, _transports, server
...
def test_version_flag(capsys: pytest.CaptureFixture[str]) -> None:
    with pytest.raises(SystemExit) as info:
        _cli.serve("srv", ["--version"], "iso20022-evidence-pack-mcp", "9.9.9")
    assert info.value.code == 0
    assert "iso20022-evidence-pack-mcp 9.9.9" in capsys.readouterr().out
