...
static dev-mode bearer token, `X-MCP-Tenant` tenant scoping) for shared
...
scout that speaks HTTP, needs the same command line on every server. The
MCP specification now has two current revisions on the HTTP binding,
...
Option 3. `iso20022-evidence-pack-mcp` runs stdio; `iso20022-evidence-pack-mcp --transport
...
`iso20022-evidence-pack-mcp --transport sse` serves the older HTTP+SSE transport at
